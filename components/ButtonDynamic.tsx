...
export default function ButtonDynamic({
  ...
  const colorVary = twcolor == "transparent"
    ? "border-transparent bg-transparent"
    : twcolor == "white"
    ? "border-white bg-white"
    : `border-${twcolor}-500 bg-${twcolor}-500`;

  const colorDarkenModVary = twcolor == "transparent"
  ...
  const colorHighlightModVary = twcolor == "transparent"
  ...
  const colorDisabledVary =
    `disabled:border-${twcolor}-300 disabled:bg-${twcolor}-300`;

  const textColorVary = textcolor == "white"
    ? "text-white"
    : `text-${textcolor}-500`;
